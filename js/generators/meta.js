function applyMeta(premises, relationFinder) {
    ...
    while (_premises.picked.length) {
        ...
        const chosenPair = pickRandomItems(_premises.picked, 2);
        const negations = chosenPair.picked.map(p => /is-negated/.test(p));
        const relations = chosenPair.picked.map(relationFinder);
